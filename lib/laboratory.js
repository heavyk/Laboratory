var Fs, Path, p$, Ini, Github, Program, Walk, LiveScript, LSAst, MachineShop, ref$, _, ToolShed, Fsm, Debug, _log, echo, debug, LAB_PATH, LAB_CONFIG_PATH, testing_repos, Laboratory, Project, SrcDir, Src, out$ = typeof exports != 'undefined' && exports || this;
Fs = require('fs');
Path = require('path');
...
testing_repos = ['Laboratory', 'Archivista', 'Mental', 'Upgrader', 'Blueshift', 'node-sencillo', 'MachineShop'];
          ...
          if (!user) {
            ...
            Fs.watch(path, function(evt, filename){
              ...
              if (evt === 'change') {
                ...
                return console.log("rename event", arguments);
              }
            });
            walker = Walk(user.path, {
              max_depth: 1
            });
...
Project = function(opts, refs){
  var path, lab, src_dir, lib_dir, prj;
  if (!opts.path) {
  ...
  prj = new Fsm({
    ...
    states: {
      uninitialized: {
        _onEnter: function(){
          var pkg_src_path, pkg_json_path;
          pkg_src_path = Path.join(path, "package.json.ls");
          pkg_json_path = Path.join(path, "package.json");
          return ToolShed.readFile(pkg_json_path, 'utf-8', function(err, data){
            var pkg_src, json, e;
            if (err) {
              if (err.code === 'ENOENT') {
              ...
              return Fs.stat(pkg_src_path, function(err, st){
                var pkg_src;
                if (!(err && st.isFile())) {
                  pkg_src = Src({
                    path: pkg_src_path,
      ...
      ready: {
        _onEnter: function(){
          lab.prjs.push(prj);
          lab.emit('added', prj);
          return console.log("totally ready");
        }
      }
    }
  });
      ...
      walk: {
        _onEnter: function(){
          ...
          return d.on('directory', function(path, st){
            ...
            console.log("we have a directory!!", arguments);
